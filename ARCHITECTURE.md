...
Validierung läuft zentral über **FluentValidation** im Projekt **CashflowSimulator.Validation**. Desktop und Engine nutzen dieselben Regeln; Fehler werden in der Desktop-App im zentralen Meldungsbereich der Shell angezeigt. Keine Validierung im XAML. Details in **`.antigravity/rules/main.md`**.
...
Technische Architektur, Datenmodell (`SimulationProjectDto`, inkl. `UiSettings` für UI-Optionen pro Szenario) und Datenfluss sind in **`.antigravity/rules/main.md`** beschrieben.
