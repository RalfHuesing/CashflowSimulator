...
- **Listen-Ansichten mit Bearbeitungsmaske** (Liste oben, Formular unten): Es **muss** das **MasterDetailView** (Common/Controls) verwendet werden. Es wird **innerhalb** von FeatureLayoutView als Content eingesetzt; **MasterContent** = Liste/Tabelle (z. B. DataGrid), **DetailContent** = Formular. Die Aktionen (Neu, Speichern, Löschen) werden über **FeatureLayoutView.ActionContent** als Toolbar-Buttons im dunklen Page-Header platziert und an das Feature-ViewModel gebunden (z. B. `Command="{Binding NewCommand}"`). Der Detail-Bereich bleibt immer sichtbar (kein Ausblenden bei leerer Auswahl).
